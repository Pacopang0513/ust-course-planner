...
- 1. **提供登录信息（一键方式，推荐）**：
  - 一次性安装：浏览器打开"扩展管理"（Chrome 输 `chrome://extensions`，Edge 输
    ...
    选择项目内 `extensions/ust-cookie` 文件夹；（此后若项目更新了扩展代码，
    需到扩展管理页点该扩展的"重新加载"才生效）
  - 之后每次排课：对 AI 说"帮我排课" → AI 会列出安装与登录步骤清单，
    并**同时告诉你端口（默认 8765）与连接码**，你照做（扩展里先填好
    端口与连接码并保存）并回复"准备好了" → AI 才启动接收端 →
    分别在已登录的 SIS（ustportal/学号登录）与 ust.space 页面各点一下
    扩展按钮即可，无需复制粘贴任何内容（连接码由 AI 侧提前生成并告知，
    首次填入扩展一次）。
