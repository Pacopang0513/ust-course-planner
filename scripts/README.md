...
│   ├── crawler.py              #   全量抓取 → data/courses_{session}.json（导师/Quota/pre-req；latest_session 自动检测）
...
│   ├── buckets.py              #   step1 未修计算（bucket 化；track 过滤 + 已修/预选扣除 + CC 三层判定）
...
│   ├── planner.py              #   step6 课程表编排（必修强制 / TBA 计学分 / must-take / waiver）
...
- 运行时产物：`data/unmet_courses.json`、`data/filter_report.json`、
  `data/review_summary.json`、`data/course_scores.json`、`output/timetable_plan.json`、
  `output/enroll_cart.json`
